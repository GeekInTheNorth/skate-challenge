...
public class StatisticLeaderQueryHandler : IRequestHandler<StatisticLeaderQuery, IEnumerable<SkaterStatisticsModel>>
{
        ...
        switch (request.StatisticType)
        {
            case StatisticType.BestAverageSpeed:
                skaterSummaries.AddRange(allSkaterSummaries.Where(x => x.BestAverageSpeed > 0)
                                                           ...
                                                           .Select(x => new Tuple<string, string>(x.ApplicationUserId, GetSpeedString(x.BestAverageSpeed))));
                break;
            case StatisticType.BestTopSpeed:
                skaterSummaries.AddRange(allSkaterSummaries.Where(x => x.BestTopSpeed > 0)
                                                           ...
                                                           .Select(x => new Tuple<string, string>(x.ApplicationUserId, GetSpeedString(x.BestTopSpeed))));
                break;
            ...
            case StatisticType.GreatestElevationGain:
                skaterSummaries.AddRange(allSkaterSummaries.Where(x => x.GreatestClimb > 0)
                                                           ...
                                                           .Select(x => new Tuple<string, string>(x.ApplicationUserId, GetElevationString(x.GreatestClimb))));
                break;
            case StatisticType.HighestElevation:
                skaterSummaries.AddRange(allSkaterSummaries.Where(x => x.SkybornSkater > 0)
                                                           ...
                                                           .Select(x => new Tuple<string, string>(x.ApplicationUserId, GetElevationString(x.SkybornSkater))));
                break;
    ...
    private static string GetDistanceString(decimal distance)
    {
        var kilometres = Conversion.MilesToKilometres(distance);

        return distance < 0.1M ? $"{Conversion.KilometresToMetres(kilometres):F2} metres" : $"{kilometres:F2} kilometres";
    }

    private static string GetSpeedString(decimal speed)
    {
        return $"{Conversion.MilesPerHourToKilometresPerHour(speed):F2} KPH";
    }

    private static string GetElevationString(decimal elevation)
    {
        return $"{Conversion.FeetToMetres(elevation):F2} metres";
    }
}